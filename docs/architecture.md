...
Claude Code also applies `availableModels` to subagent definitions. In 2.1.207, an excluded custom gateway id silently falls back to the parent model. remora supplies its configured ids as an additional session-only allowlist. When a caller also passes one `--settings` JSON file or inline object, remora recursively merges the caller document first and then overlays its routing allowlist. Before serialization it requires `env` to be an object and removes keys owned by remora's child gateway/model/session contract, preserving unrelated caller variables. The merged document is written to a unique `0600` temporary file because Claude Code resolves `/dev/fd/N` back to the original pathname and rejects an already unlinked source. Before payload writing begins, remora starts a detached cleanup watcher and retains the write end of a guard pipe across `execvpe`; normal exit, failed launch, or process termination closes that pipe and removes the file. Parent-only path and guard metadata are stripped from the Claude environment. This keeps caller hooks active without losing routing control or placing the full document in the Claude child arguments. It is a compatibility allowance, not a bypass of higher-precedence managed policy.
...
Plan and outcome verification use deliberately different roles, capabilities, and vocabularies. For long-running or large work, the program envelope owns outcome/non-goals, cross-cutting architecture/security/privacy invariants, the dependency DAG, integration/rollback, and global budget/stops. It has its own stable readiness-unit ID and must receive a fresh `READY` verdict before the main session reviews any child slice. The ready envelope then decomposes into the smallest genuinely independently approvable, executable, and verifiable slices. Each slice carries a stable ID, exclusive ownership, stable prerequisites, acceptance, and rollback, and cosmetic fragmentation cannot bypass a blocker. The read-only `plan-verifier` reviews exactly one envelope or slice per call and returns the bare word `READY`, or a structured non-bare `REVISE` in which every blocker includes `Blocker`, `Evidence` (a `file:line` citation or explicit evidence gap), `Minimum revision`, and `Acceptance check`; the read-and-run `verifier` returns `CONFIRMED`, `REFUTED`, or `INCONCLUSIVE`. Decorated or malformed output is a protocol failure rather than a Plan judgment: one fresh same-unit format-recovery retry is allowed per epoch without revising the Plan or consuming a valid automatic `REVISE` round. After the envelope and next executable slice are `READY`, the main session stops readiness review and seeks explicit approval. Only that next slice is fully specified; downstream entries retain stable IDs, outcomes, dependency and ownership boundaries, acceptance intent, and rollback/stop summaries until they become next, unless the user requests a batch or one approval and integration boundary requires them together. Readiness verdicts, Plan epochs, and the automatic count are tracked per readiness-unit ID. Each automatic `REVISE` requires a material revision, genuine split/narrowing, or new evidence and a fresh plan-verifier; after two automatic `REVISE` verdicts in one unit epoch, only that unit pauses and asks the user to choose how to continue. A paused envelope blocks every child. Child slices get epochs only when dependencies and acceptance are independently meaningful. A `READY` slice may be approved and executed while unrelated slices remain in planning, but paused prerequisites and cross-cutting blockers still gate dependents. Only a material user change to a unit's outcome, scope, or architecture, or an explicitly selected materially new envelope/slice Plan, starts a new epoch; superficial rewrites cannot reset the cap. `READY` is readiness only, never approval. For security-sensitive units, read-only `security-reviewer` evidence must be carried into the slice Plan, envelope, and ledger before the first `plan-verifier` call, and those reviews cannot run concurrently; explicit user approval still gates implementation.

Outcome verification is calibrated to the exact acceptance and regressions caused by the reviewed implementation. `CONFIRMED` requires evidence for every acceptance criterion; `REFUTED` requires a reproducible P0-P2 blocker and takes precedence over missing evidence for another criterion; without such a blocker, an unevaluated required criterion produces `INCONCLUSIVE`. P3/P4 remain advisory. P0 freezes the affected slice, and introduced P2 regressions remain blocking. Every verification run shares five materially changed P1/P2 recovery passes and pauses only after the fifth remains unsuccessful or blocking. Verification identity includes the complete tested candidate, claim, acceptance, contract, external evidence or prerequisites, and environment; a prior verifier's own output is not a change. The candidate fingerprint covers committed head, tracked and staged diff, untracked input paths plus content, and dirty submodule content. Artifact digests complement source identity unless the artifact is the sole deliverable. Long autonomous work declares `AUTO` or `ASK`; neither mode expands user authority.
...
Fresh outcome verification runs at the smallest coherent integration boundary where the complete slice claim can be refuted. Tests and builds are intermediate evidence, while security, FFI, serialization, pre-aggregation, irreversible, and integration-blocking changes verify earlier. A readiness pass is repeated only after a material slice revision, genuine split/narrowing, or new evidence; persistent disagreement is simplified, surfaced, or deferred rather than silently overruled, and a slice's two-verdict automatic cap is never treated as `READY`. Completed recon output is collected from the tracked task, and continuation is reserved for liveness, redirection, or genuinely new work.
