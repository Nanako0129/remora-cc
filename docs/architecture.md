...
Outcome verification is calibrated to the exact acceptance and regressions caused by the reviewed implementation. `REFUTED` requires a reproducible P0-P2 blocker; P3/P4 remain advisory, while unavailable or unsafe evidence produces `INCONCLUSIVE`. P0 freezes the affected slice, and introduced P2 regressions remain blocking. Blocking P1/P2 recovery shares five materially changed passes. Verification identity includes acceptance, while candidate identity includes committed head plus working-tree diff or tested-artifact digest, so uncommitted fixes can be rechecked without repeating an unchanged candidate. Long autonomous work declares `AUTO` or `ASK`; neither mode expands user authority.
