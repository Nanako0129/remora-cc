...
| Additional settings | 無 | Inline routing allowlist 與 no-fallback policy，或透過受監控的 `0600` 暫存檔傳入合併後的 caller 文件 |
...
| `plan-verifier` | `gpt-5.6-sol` | medium | 批准前、由工具限制為唯讀的 Plan 挑戰 |
| `security-reviewer` | `gpt-5.6-sol` | high | 批准前、由工具限制為唯讀的安全證據檢查 |
...
Claude Code 會用 `availableModels` 驗證 subagent model。若使用者平常的設定只列 Claude aliases，`gpt-5.6-luna` 這類 gateway id 原本會被拒絕，並且靜默繼承 Sol main model。remora 會傳入只存在 child 的 additional settings，allowlist 所有已設定的 gateway model，並注入精確的 `fallbackModel: []`；因此 remora main session 不會自動落到從 user、project 或 local settings 繼承的 fallback model。這項行為已針對 Claude Code 2.1.216 與 2.1.217 做過相容性測試。明確選擇 model 仍然有效：可用 `remora --model sonnet`、`remora -m sonnet` 或 `remora --model=gpt-5.6-terra` 選擇 Terra。

若 caller 也以 JSON 檔案或 inline object 傳入 `--settings`，remora 會合併兩個來源、移除 caller `env` 中由 remora 掌管的 gateway／model／session keys，並保留其他 caller 設定。Caller 若帶有 `fallbackModel`，其值必須是只含非空白字串的 JSON array（空 array 合法）；remora 會在不回顯內容的前提下驗證、移除它，再注入 authoritative empty array。等價的 Claude CLI `--fallback-model` 會在 gateway authentication、context discovery、暫存檔建立或 child execution 之前被拒絕。合併後的 caller 文件會寫入唯一的 `0600` 暫存檔，另由獨立的 pipe watcher 在 Claude process 結束或被終止時移除，因此完整 JSON 與 secret 不會出現在 child argv 或 dry-run；只有 routing settings 時仍維持 inline。Caller settings 含有 secret 時應使用 JSON 檔案，不要使用 inline JSON，因為 inline 值原本就會出現在 remora 的呼叫命令列。

這項 policy 只作用於 remora child，不會寫入或取代使用者設定。組織 managed policy 的優先權高於 remora additional settings；若組織強制自己的 fallback，remora 無法覆寫該 control，管理者必須先對齊 managed policy，才能依賴 no-fallback 行為。

Happy 這類 wrapper 也會附加自己的 system prompt。啟動 wrapper 時設定 `REMORA_COMPOSE_SYSTEM_PROMPT=1`，remora 會把 caller 的 inline／file prompt 放前面、自己的 orchestration policy 放後面，合成單一 child-only `--append-system-prompt`。分開傳入的 prompt 值或 prompt-file 路徑可以 `-` 開頭，`--` 仍會結束 remora 的 option 掃描。Happy 的 Agent SDK adapter 會自動把 remote append prompt bridge 給 remora，避免 SDK initialize protocol 覆寫 executable CLI flag。未啟用時，explicit `--append-system-prompt*` 維持既有行為，仍會取代 remora 預設 policy。

對於已命名的 agent，角色定義是唯一的 model 來源。remora 的 orchestration policy 因此禁止在呼叫 `Explore`、`scout`、`plan-verifier`、`security-reviewer`、`mech-executor`、`executor`、`verifier`、`security-executor` 或任何其他已存在的命名角色時傳入 `model`；Claude Code 會優先使用 invocation-level model，進而繞過角色表。只有沒有角色定義的真正 ad-hoc agent 才會在呼叫時明確指定 model。
...
| Secret 最小化 | Gateway token 不會印出；合併後的 caller settings 使用受監控的 `0600` 暫存檔而非 child argv，caller settings 也無法覆寫 remora-owned environment keys |
...
八角色版本之前建立的設定仍可使用：缺少 `plan-verifier` 時會沿用 `verifier` 路由，缺少 `security-reviewer` 時則沿用 `security-executor`。若要分別控制它們的 effort，請依 `config.example.toml` 補上明確欄位。
...
remora 不認識的參數會原樣交給 `claude`。若明確傳入 `--model` 或 `--agents`，該項會以你的參數為準，不再注入 remora default。刻意的例外是 `--fallback-model`：remora 會在 launch 前拒絕它，以維持停用自動 fallback；`--` delimiter 後的文字則完全不動。

### Fast 模式

Fast 模式是明確 opt-in、只作用於單一 session。將 `--fast` 放在 wrapper
名稱後的第一個位置（或 `dry-run` 後的第一個位置），即可讓該 child session
的所有 request 使用 `service_tier=priority`：
...
預設關閉、不會持久化，而且可能增加 provider credit／usage。設定的
Anthropic-compatible gateway 必須接受並轉送這個 service tier；remora 只在
child request body 注入它。已驗證 stock CLIProxyAPI v7.2.80；這只是相容性檢查，
不是最低版本宣告，Fast 也不會繞過 provider quota。用 `dry-run` 語法可驗證
合成後的 child 設定：preview 只顯示 `{"service_tier":"priority"}`，並遮蔽繼承
的其他欄位。
...
| remora 能與 Baton 這類 delegation-planning skill 一起使用嗎？ | 可以，前提是責任合成而非互相覆寫：Baton 決定 Discovery 與 Execution 拓撲；remora 提供命名角色、模型路由、leaf 邊界、批准，以及分離的 Plan／outcome verifier 角色。[v0.1.8 相容性 Gate](./benchmarks/baton-compatibility/README.zh-TW.md) 已完成 Discovery → main-session Plan → 明確批准 → Execution → fresh verification，探索實際使用 Luna，驗證使用 Sol。remora 不會暗中停用任意 skill；explicit `--append-system-prompt*` 預設仍取代 policy，只有 `REMORA_COMPOSE_SYSTEM_PROMPT=1` 會合成兩個來源。 |
...
| `--settings must reference ...` | 傳入的 settings 路徑無法讀取、內容不是單一有效 JSON object，或 `env` 不是 object | 修正輸入格式；remora 接受一個來源、移除 remora-owned `env` keys，再以受監控的 `0600` 暫存檔傳入合併結果 |
| Wrapper 的 prompt 蓋掉 remora orchestration | Wrapper 傳入 explicit append prompt，預設會取代 remora policy | 以 `REMORA_COMPOSE_SYSTEM_PROMPT=1` 啟動；remora 會依序轉送 caller 內容與自己的 policy |
