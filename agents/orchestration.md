...
After the dispatch brake establishes that delegation's net benefit remains positive, launch every eligible delegation with `run_in_background: true`, including a single agent whose result is the next dependency. When dispatching two or more independent agents, launch them as one parallel background batch. Give each writing agent an isolated worktree and integrate its changes after completion; read-only agents may share the checkout. If the next action depends on an agent, launch it in the background and then wait for and collect it instead of switching that delegation to foreground execution. Do not launch an agent merely to wait for it when the main session already owns the same evolving evidence and can finish more cheaply overall.

- If the main session can make useful progress before an agent returns, continue that work.
...
Long-running processes belong to the main session and must use the execution tool's native background mode. The main session records the returned task or session handle, continues independent work when useful, and collects completion before any dependent action or final answer. Leaf agents must not launch or detach long-running processes; they return the exact command, absolute working directory or isolated workspace, required environment, input paths, and completion criterion so the main session can launch it in the background and resume the agent with the captured result. Never substitute shell detachment such as `&`, `nohup`, or `disown` for tool-native background execution. If the required background capability is unavailable, pause and report the blocker instead of silently running the command in the foreground or detaching it.
...
Do not resubmit a substantially unchanged slice Plan to `plan-verifier`. After two automatic verdicts, use the main-session `FIX`/`DEFER`/`REJECT` disposition, simplify or narrow the unit, and continue unrelated approved slices. Ask only for an unresolved P0/P1, product or authority choice, or unattainable original scope; never treat the budget cap as `READY`.
