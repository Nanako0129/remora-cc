...
implementation-caused regressions. Verifiers now report `CONFIRMED`, `REFUTED`,
or `INCONCLUSIVE` with P0-P4 findings. Only reproducible P0-P2 blockers can
refute a claim; the main session evaluates severity and owns final disposition.

Blocking P1/P2 recovery has a shared five-pass budget and requires materially
changed candidate evidence before another verification pass. Candidate identity
covers tracked, staged, untracked, and submodule state so unchanged retries
cannot consume a long autonomous run. `AUTO` and `ASK` define whether the main
session may apply bounded fixes or must pause; neither mode grants new authority.

## 0.1.15 - 2026-07-24

Align the session-only orchestration policy with the bounded slice Plan-readiness contract. Long-running or large work now starts with a stable-ID program envelope for outcome/non-goals, cross-cutting architecture/security/privacy invariants, dependencies, integration/rollback, and global budget/stops. A fresh `plan-verifier` reviews that envelope alone, and it must be `READY` before any child slice is reviewed; each later call likewise reviews exactly one stable-ID slice. Ready envelopes decompose into independently approvable, executable, and verifiable slices with ownership, prerequisites, acceptance, and rollback. After the envelope and next executable slice reach `READY`, readiness review stops for explicit approval; downstream slices keep stable boundaries and acceptance intent but defer implementation detail unless the user requests a batch or one approval and integration boundary requires them together. `plan-verifier` returns only a bare `READY` or a structured `REVISE` with `Blocker`, `Evidence`, `Minimum revision`, and `Acceptance check` for every blocker. Decorated or malformed output is a protocol failure rather than a Plan judgment; it permits one fresh same-unit format-recovery retry per epoch without consuming either valid automatic `REVISE` round. Each readiness-unit epoch permits at most two automatic `REVISE` rounds; after the second verdict only that unit pauses and surfaces blockers/options for explicit user direction, while unrelated approved `READY` slices may proceed. A paused envelope blocks every child. Cosmetic splits and superficial rewrites cannot bypass blockers or reset an epoch; material unit changes or an explicitly selected new envelope/slice Plan can. Security-sensitive units require completed read-only `security-reviewer` evidence before the first `plan-verifier` call and the reviews cannot run concurrently. `READY` remains separate from user approval and `CONFIRMED`/`REFUTED` outcome verification. Routing, session isolation, and fallback suppression are unchanged.
