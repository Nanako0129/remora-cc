...
| Additional settings | 無 | Inline routing allowlist，或透過受監控的 `0600` 暫存檔傳入合併後的 caller 文件 |
...
Claude Code 會用 `availableModels` 驗證 subagent model。若使用者平常的設定只列 Claude aliases，`gpt-5.6-luna` 這類 gateway id 原本會被拒絕，並且靜默繼承 Sol main model。remora 現在會傳入只存在 child 的 additional settings，allowlist 所有已設定的 gateway model；若 caller 也以 JSON 檔案或 inline object 傳入 `--settings`，remora 會合併兩個來源、移除 caller `env` 中由 remora 掌管的 gateway／model／session keys，並保留其他 caller 設定。合併後的 caller 文件會寫入唯一的 `0600` 暫存檔，另由獨立的 pipe watcher 在 Claude process 結束或被終止時移除，因此完整 JSON 與 secret 不會出現在 child argv 或 dry-run；只有 routing settings 時仍維持 inline。Caller settings 含有 secret 時應使用 JSON 檔案，不要使用 inline JSON，因為 inline 值原本就會出現在 remora 的呼叫命令列。remora 不寫入或取代使用者設定，組織 managed policy 也仍保有原本的較高優先權。
...
```text
請閱讀並遵循這份安裝 runbook：
https://raw.githubusercontent.com/Nanako0129/remora-cc/v0.1.12/install/AGENT-INSTALL.md
...
```bash
git clone --branch v0.1.12 --depth 1 https://github.com/Nanako0129/remora-cc.git
cd remora-cc
./install.sh
```
