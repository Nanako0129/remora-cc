...
remora 只替一個 child `claude` process 注入 `--agents` JSON、session-only 編排補充指令與 gateway 環境變數。Claude Code 官方把 `--agents` 定義為只存在於當前 session 的 agent source；它可以暫時覆蓋同名的 project/user agent，但不會寫入磁碟。編排補充指令會把可獨立推進的工作放到背景，只有主流程下一步立即依賴結果時才使用前景 agent。
...
OpenAI 公開 GPT-5.6 API 標示 1.05M context window，但 CLIProxyAPI 的 Codex OAuth catalog 目前替 Sol、Terra、Luna 回報 372K。remora 以 gateway catalog 作為實際運作上限，不假設公開 API 與訂閱／OAuth 路徑具有相同 ceiling。
...
想使用完整 provider window 的人，可以自行安裝 [Calico Claude](https://github.com/Nanako0129/calico-claude)，再明確選擇 `calico` 模式。remora 會讀取 gateway catalog，將逐一對應的 model/window map 交給 Calico 的驗證型 context adapter；畫面以 95% 顯示 usable context，compact 則使用 raw window 的 90%。372K 因此顯示 353.4K usable context，約 334.8K compact。Map 格式錯誤、model id 不吻合或 binary 缺少 patch 時都會 fail closed。

| 模式 | Claude binary | 顯示視窗 | Compact 時機 | 預設 |
|---|---|---:|---:|---|
| `stock` | 官方 native Claude Code | 200K | Claude 原生管理 | 是 |
| `calico` | 通過驗證的 Calico release | Provider 的 95% | Provider 的 90% | 明確 opt-in |
...
| `Your input exceeds the context window` | 選擇的 client mode 與 provider ceiling 不一致 | 執行 `remora doctor --online`；使用 200K `stock` 預設，或先安裝 Calico 再選 `calico` |
