...
| Runtime policy | Read `agents/agents.json` and `agents/orchestration.md` from the same immutable tag; report that the latter is appended only to the remora child session and, when the independent-review trigger applies, gates the stable-ID program envelope before independently gated slices and enforces exactly-one-unit bare `READY`/structured `REVISE` readiness with a two-verdict automatic brake per unit epoch; it controls foreground/background delegation and leaves every named role's model to its agent definition |
...
Confirm that `remora doctor` prints every configured GPT model under `PASS routing allowlist`, and that `remora dry-run` contains the reviewed `--append-system-prompt` policy without exposing the gateway token. A caller may pass one `--settings` JSON file or inline object; remora merges it with the routing allowlist, removes remora-owned keys from caller settings `env`, and preserves unrelated caller settings. Caller-supplied documents must appear in dry-run only as a unique temporary path, never as full JSON or secret values; the referenced file must be `0600`, and a detached guard removes it when the Claude process exits or is killed. Routing-only settings may remain inline. Settings containing secrets should use a JSON file rather than inline JSON, which is already visible in the original remora command line. For wrappers that always append their own prompt, set `REMORA_COMPOSE_SYSTEM_PROMPT=1`; dry-run must then show one inline prompt with caller content first and remora policy second. Separated prompt values and prompt-file paths may begin with `-`, while `--` remains the option delimiter. Agent SDK wrappers must bridge their initialize-protocol append prompt through child-only `REMORA_CALLER_SYSTEM_PROMPT`; remora consumes and removes it before runtime launch. Without the opt-in, explicit prompt arguments continue to replace the default.
