...
Outcome verification is calibrated to the exact acceptance and regressions caused by the reviewed implementation. `CONFIRMED` requires evidence for every acceptance criterion; `REFUTED` requires a reproducible P0-P2 blocker and takes precedence over missing evidence for another criterion; without such a blocker, an unevaluated required criterion produces `INCONCLUSIVE`. P3/P4 remain advisory. P0 freezes the affected slice, and introduced P2 regressions remain blocking. Blocking P1/P2 recovery shares five materially changed passes and pauses only after the fifth remains unsuccessful or blocking. Verification identity includes the complete tested candidate, claim, acceptance, contract, available evidence or prerequisites, and environment. The candidate fingerprint covers committed head, tracked and staged diff, and untracked input paths plus content, or a tested-artifact digest. Long autonomous work declares `AUTO` or `ASK`; neither mode expands user authority.
