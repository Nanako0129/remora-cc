...
Claude Code 會用 `availableModels` 驗證 subagent model。若使用者平常的設定只列 Claude aliases，`gpt-5.6-luna` 這類 gateway id 原本會被拒絕，並且靜默繼承 Sol main model。remora 現在會傳入只存在 child 的 additional settings，allowlist 所有已設定的 gateway model；若 caller 也以 JSON 檔案或 inline object 傳入 `--settings`，remora 會合併成同一份 additional settings，不會捨棄任一來源。它不寫入或取代使用者設定，組織 managed policy 也仍保有原本的較高優先權。

Happy 這類 wrapper 也會附加自己的 system prompt。啟動 wrapper 時設定 `REMORA_COMPOSE_SYSTEM_PROMPT=1`，remora 會把 caller 的 inline／file prompt 放前面、自己的 orchestration policy 放後面，合成單一 child-only `--append-system-prompt`。Happy 的 Agent SDK adapter 會自動把 remote append prompt bridge 給 remora，避免 SDK initialize protocol 覆寫 executable CLI flag。未啟用時，explicit `--append-system-prompt*` 維持既有行為，仍會取代 remora 預設 policy。
...
| `--settings must reference ...` | 傳入的 settings 路徑無法讀取，或內容不是單一有效 JSON object | 修正檔案路徑或改傳 inline object；remora 接受一個 `--settings` 來源並與 routing allowlist 合併 |
